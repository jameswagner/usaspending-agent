...
NEVER_CHART_TOOLS = {
    ...
    # A single scoped total (plus a small per-DEFC funding list, at most
    # ~40 rows), same "one profile, no real cardinality to chart" reasoning
    # as get_agency_budget/get_award_details above, not a ranked/
    # categorical breakdown like get_spending_by_category.
...
def build_tool_citation(tool_name: str, context: dict, result=None) -> ToolCitation | None:
    ...
    if tool_name == "get_disaster_spending_overview":
        ...
        # Comma-joined, matching the client's own wire format for this
        # endpoint - see USASpendingClient.get_disaster_overview's
        # docstring for why a plain repeated-param list silently drops
        # all but the last code here.
        if def_codes:
    ...
    if tool_name == "get_spending_explorer_breakdown":
        ...
        for key in (
            "agency", "budget_function", "budget_subfunction",
            "federal_account", "object_class", "recipient", "program_activity",
        ):
            if context.get(key):
                params[key] = context[key]
        description = (
            f"Spending explorer, group by {params['group_by']}, FY{params['fiscal_year']} Q{params['quarter']}"
        )
        return ToolCitation(
            tool_name=tool_name, parameters=params, description=description, curl=_curl_from_context(context)
        )
