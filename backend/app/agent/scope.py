...
def _tag_scope_decision(in_scope: bool, used_followup_prompt: bool) -> None:
    """Enriches the already-@traceable scope_classifier run with this
    app's own outcome vocabulary - LangSmith has the span/latency/token
    data already (via wrap_anthropic), just not what the classifier
    actually decided, or which of the two prompts (fresh vs. follow-up)
    it used to decide it.
    """
    run_tree = get_current_run_tree()
    if run_tree is not None:
        run_tree.add_metadata({"in_scope": in_scope, "used_followup_prompt": used_followup_prompt})
...
def _is_in_scope(question: str, recent_messages: list | None = None) -> bool:
    ...
    if recent_messages:
        ...
        if history_block:
            ...
            _tag_scope_decision(decision, used_followup_prompt=True)
            return decision
    ...
    text = next((b.text for b in response.content if b.type == "text"), "")
    decision = text.strip().upper().startswith("YES")
    _tag_scope_decision(decision, used_followup_prompt=False)
    return decision
