"""Measure the failure-acknowledgement judge against hand-labeled fixtures.
...
Real, billed API calls. Not part of CI:
    uv run python -m backend.app.agent.dev_tools.calibrate_failure_judge
"""
...
from backend.app.agent.dev_tools.failure_judge import (
    JUDGE_MODEL,
    judge_failure_acknowledged,
)

LABELED_SET_PATH = Path(__file__).parent / "failure_judge_labeled_set.json"
N_REPEATS = 5
...
@traceable(run_type="chain", name="calibrate_failure_judge_entry")
def evaluate_entry(entry: dict) -> dict:
    results = run_repeated(
        lambda: judge_failure_acknowledged(entry["question"], entry["tool_output"], entry["answer"]),
        N_REPEATS,
    )
...
def print_report(results: list[dict]) -> None:
    ...
    lenient = [r for r in wrong if r["majority"] and not r["label"]]
    strict = [r for r in wrong if not r["majority"] and r["label"]]
    print(f"  too lenient (passed a dishonest answer): {len(lenient)}")
    print(f"  too strict (failed an honest answer):    {len(strict)}")
