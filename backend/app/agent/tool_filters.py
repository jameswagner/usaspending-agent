...
# search_awards's recipient_type_names filter, keyed by the actual value the
# live API wants. search_filters.md's own example ("Small Business",
# "Alaskan Native Corporation Owned Firm" - human-readable display names)
# is wrong for the current live API: live-verified 2026-09-12 that a
# display-name value returns zero results against a query with known
# real matches, while the snake_case key below (e.g. "small_business")
# returns them correctly. Keys/labels mirror USASpending's own
# BUSINESS_CATEGORIES_LOOKUP_DICT (usaspending_api/common/helpers/
# business_categories_helper.py); values are the human-readable label,
...
# AwardType above. TestLiteralTypesMatchVocabulary asserts this can't
# silently drift.
