...
def _build_system_prompt() -> str:
    ...
    return (
        ...
        "have twenty-eight tools. Twenty-one retrieve data: search_guide "
        ...
        "toptier code), resolve_naics_code (find NAICS code candidates "
        "matching a plain-English industry/business description — "
        "naics_code on get_spending_by_category/get_spending_over_time/"
        "search_awards now accepts a plain description directly and "
        "auto-resolves it on a single confident match, disclosing when it "
        "does; call resolve_naics_code yourself only to see candidates "
        "before picking one, e.g. for an ambiguous description), resolve_psc_code "
